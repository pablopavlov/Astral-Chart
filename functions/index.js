...
// Importa la librería 'astrology' para realizar cálculos astrológicos.
...
exports.generateAstralChart = functions.https.onRequest(async (req, res) => {
    ...
    // Si usas Firebase Hosting para el frontend, Firebase ya maneja CORS internamente,
    // pero incluirlos aquí es una buena práctica y para depuración.
    ...
    // El navegador envía una solicitud OPTIONS antes de la solicitud POST real para verificar los permisos.
    ...
    try {
        ...
        // La librería 'astrology' espera la hora local, no la UTC. La corrección se hará en la librería misma.
        // Aquí usamos localDate directamente para los cálculos.
        ...
        const astrology = new Astrology({
            houseSystem: "placidus" // Sistema de casas común en astrología occidental
        });
        ...
        // Mapear la respuesta de la librería 'astrology' a un formato legible para Gemini
        const planetsData = Object.keys(chart.planets).map(key => {
            ...
            const houseNumber = planet.house ? planet.house.toFixed(0) : 'N/A';
        ...
        const housesData = Object.keys(chart.houses).map(key => {
            ...
            return `Casa ${house.name} en ${signName} a ${degree}`;
        }).join(', ');

        // La librería no calcula aspectos de texto, Gemini los interpretará.
        ...
        const finalAstroData = {
            ...
            sunSign: chart.planets.sun ? chart.planets.sun.sign.name : 'Desconocido',
            moonSign: chart.planets.moon ? chart.planets.moon.sign.name : 'Desconocido',
            ascendant: chart.houses.house1 ? chart.houses.house1.sign.name : 'Desconocido',
